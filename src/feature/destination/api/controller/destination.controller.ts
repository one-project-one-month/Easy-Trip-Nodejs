...
class DestinationController {
    ...
    async getDescription(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const data = await scoreCalculationUseCase.incrementScore({
                destinationId: destination_id, step: "step_2"
            });
            res.status(200).json({
                content: data
            });
    ...
    async getPopularDestination(req: Request, res: Response, next: NextFunction) {
        try {
            const data = await destinationUseCase.getPopularDestination({ limit: 6, orderBy: 'desc' });
            res.status(200).json({
                content: data
            });
        } catch (err) {
